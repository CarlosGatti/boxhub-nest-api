...
const BUCKET_MEDIA_ALLOWED_TYPES = [
  'image/jpeg',
  'image/png',
  'image/gif',
  'image/webp',
...
const createMulterOptions = (folder: string, allowedMimeTypes?: string[]) => {
  ...
  return {
    ...
    fileFilter: (req: any, file: any, cb: any) => {
      const allowedTypes = allowedMimeTypes ?? ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
...
export class DiscartItemUploadController {
  ...
  /**
   * Upload cover image for a bucket goal (Vision Board).
   * Single image, processed with Sharp (resize/compress).
   */
  @Post('bucket-goals/cover')
  @UseGuards(JwtAuthGuard)
  @UseInterceptors(FileInterceptor('file', createMulterOptions('bucket-goals/cover')))
  async uploadBucketGoalCover(@UploadedFile() file: Express.Multer['File']) {
    if (!file) {
      throw new BadRequestException('No file uploaded');
    }

    try {
      const filePath = join(process.cwd(), 'uploads', 'bucket-goals', 'cover', file.filename);

      if (!existsSync(filePath)) {
        return { url: `/uploads/bucket-goals/cover/${file.filename}` };
      }
      ...
      try {
        ...
        if (existsSync(outputPath)) {
          ...
          const finalFilename = file.filename.replace(extname(file.filename), '.jpg');
          return { url: `/uploads/bucket-goals/cover/${finalFilename}` };
        }
        return { url: `/uploads/bucket-goals/cover/${file.filename}` };
      } catch (processError: any) {
        console.warn(`⚠️ Bucket cover processing failed, using original:`, processError.message);
        return { url: `/uploads/bucket-goals/cover/${file.filename}` };
      }
    } catch (error: any) {
      if (error.code === 'EACCES' || error.code === 'EPERM') {
  ...
  /**
   * Upload media for bucket goals (memories, event photos, travel photos, etc.).
   * Multiple files. Images are processed; videos/PDFs stored as-is.
   */
  @Post('bucket-goals/media')
  @UseGuards(JwtAuthGuard)
  @UseInterceptors(
    FilesInterceptor('files', 10, createMulterOptions('bucket-goals/media', BUCKET_MEDIA_ALLOWED_TYPES)),
  )
  async uploadBucketGoalMedia(@UploadedFiles() files: Array<Express.Multer['File']>) {
    if (!files || files.length === 0) {
      throw new BadRequestException('No files uploaded');
    }
    ...
    for (const file of files) {
      try {
        const filePath = join(process.cwd(), 'uploads', 'bucket-goals', 'media', file.filename);

        if (!existsSync(filePath)) {
          processedUrls.push(`/uploads/bucket-goals/media/${file.filename}`);
          continue;
        }
        ...
        if (imageTypes.includes(file.mimetype)) {
          try {
            ...
            if (existsSync(outputPath)) {
              ...
              const finalFilename = file.filename.replace(extname(file.filename), '.jpg');
              processedUrls.push(`/uploads/bucket-goals/media/${finalFilename}`);
            } else {
              processedUrls.push(`/uploads/bucket-goals/media/${file.filename}`);
            }
          } catch (processError: any) {
            console.warn(`⚠️ Bucket media processing failed, using original:`, processError.message);
            processedUrls.push(`/uploads/bucket-goals/media/${file.filename}`);
          }
        } else {
          processedUrls.push(`/uploads/bucket-goals/media/${file.filename}`);
        ...
        if (error.code === 'EACCES' || error.code === 'EPERM') {
          throw new HttpException(`Permission denied: Cannot write to upload directory.`, HttpStatus.FORBIDDEN);
        }
        if (error.code === 'ENOSPC') {
          throw new HttpException('No space left on device', 507);
        }

import { Field } from '@nestjs/graphql';
import { InputType } from '@nestjs/graphql';
import { StringFieldUpdateOperationsInput } from '../prisma/string-field-update-operations.input';
import { NullableStringFieldUpdateOperationsInput } from '../prisma/nullable-string-field-update-operations.input';
import { EnumDefinedIntakeAnswerTypeFieldUpdateOperationsInput } from '../prisma/enum-defined-intake-answer-type-field-update-operations.input';
import { GraphQLJSON } from 'graphql-type-json';
import { DateTimeFieldUpdateOperationsInput } from '../prisma/date-time-field-update-operations.input';
import { DefinedIntakeFormUpdateOneRequiredWithoutAnswersNestedInput } from '../defined-intake-form/defined-intake-form-update-one-required-without-answers-nested.input';

@InputType()
export class DefinedIntakeAnswerUpdateInput {

    @Field(() => StringFieldUpdateOperationsInput, {nullable:true})
    questionKey?: StringFieldUpdateOperationsInput;

    @Field(() => NullableStringFieldUpdateOperationsInput, {nullable:true})
    questionLabel?: NullableStringFieldUpdateOperationsInput;

    @Field(() => EnumDefinedIntakeAnswerTypeFieldUpdateOperationsInput, {nullable:true})
    answerType?: EnumDefinedIntakeAnswerTypeFieldUpdateOperationsInput;

    @Field(() => GraphQLJSON, {nullable:true})
    answerValue?: any;

    @Field(() => DateTimeFieldUpdateOperationsInput, {nullable:true})
    createdAt?: DateTimeFieldUpdateOperationsInput;

    @Field(() => DateTimeFieldUpdateOperationsInput, {nullable:true})
    updatedAt?: DateTimeFieldUpdateOperationsInput;

    @Field(() => DefinedIntakeFormUpdateOneRequiredWithoutAnswersNestedInput, {nullable:true})
    intakeForm?: DefinedIntakeFormUpdateOneRequiredWithoutAnswersNestedInput;
}
